...
func hash_chain{hash_ptr: HashBuiltin*, range_check_ptr}(arr_ptr: felt*, arr_len: felt) -> (result: felt) {
    if(arr_len == 2) {
        return(hash2{hash_ptr=hash_ptr}(arr_ptr[0], arr_ptr[1]));
    }
    let (next) = hash_chain{hash_ptr=hash_ptr}(arr_ptr, arr_len - 1);
    return(hash2{hash_ptr=hash_ptr}(next, arr_ptr[arr_len - 1]));
